...
+ 3.3 实现细节  
我们的实现基于公开可用C++ Caffe工具箱（Jia，2013）（2013年12月的分支），但是做了一系列重要的修改，使得可以在单系统的多GPUs上训练和测试，
还可以在多尺度的训练和测试时使用全尺寸的图片。多GPUs训练可以充分利用数据并行，它是将训练图片分批输入几个GPU中，在几个GPU中并行的处理。
在GPU的批梯度计算之后，这些梯度
平均后求整个批次的梯度。因为所有GPUs上的梯度计算都是同步的，所以结果和在单GPU上训练时一样的。  
虽然近来(Krizhevsky, 2014)提出更复杂的加速卷积神经网络训练的方法，这种方法针对不同的层使用数据并行或模型并行，
而我们发现我们概念上更简单的模式已经在现成的（off-the-shelf）4GPU系统实现了3.75倍的加速，相比于单GPU。
在一个装有四块NVIDIA Titan Black GPUs的系统上，训练单个网络需要2-3周不等的时间，具体时间取决于具体网络配置。  
+ 4.分类实验  
**数据集** 在这个部分，我们将展示在ILSVRC-2012数据集上通过过上述的网络结构实现的图像分类结果（ILSVRC2012-2014都使用该数据集）。
该数据集包含1000类的图片，并且被分为训练集（1.3M张图片），验证集（50K张图片）和测试集（100K张留有标签的图片）。
分类性能使用两种方法来评估：top-1错误和top-5错误。
前者是一种多类别分类错误，换句话说，被错误分类图片的比例；后者是ILSVRC的主要评估准则，它计算的是真实标签不在预测的前5个类别的图片的比例。  
在大多数的实验中，我们把验证集当作测试集使用。
...
第二，我们发现分类错误随着网络深度的增加而减少：从11层的A配置到19层的E配置。特别地（Notably），尽管深度相同，
配置C（包含3个1x1的卷积层）的性能比配置D差，配置D在整个网络中使用3x3的卷积层。
这表明(indicates)虽然额外的非线性起了作用(配置C比配置B好)，但是通过具有有意义的(non-trivial=significant)接受域的卷积层来获得空间上下文是很重要的。
当深度到达19层，我们的网络的错误率就会饱和，但是更深的模型对更大的数据集更有好处。
我们也将配置B和包含5x5卷积核的浅层网络比较，这个浅层网络是将B配置网络的每一套3x3卷积层用单个5x5的卷积层替换（两者之间有相同的接受域，正如Sect. 2.3描述的那样）。
浅层网络的top-1错误比B配置高7%(在中心裁剪的图片上)，这也证明使用小卷积核的深层网络优于使用大卷积核的浅层网络。  
最后，尽管测试的时候使用单尺寸，但是相比于训练的时候使用固定的最小边(S=256或S=384)，训练的时候尺寸变化(jittering)($ S \in [256; 512] $)会得到更好结果。
这也证明通过尺寸变化来增广训练集对于获得多尺寸的图片统计学信息(statistics)是真的有帮助。  
+ 4.2多尺寸评估  
已经用单尺寸评估了卷积神经网络，现在我们现在要评估在测试的时候使用尺寸变化的效果。
它由运行在一张测试图片的几个重新缩放版本(对应不同的Q值)的模型组成，然后再平均得到的类后验(class posteriors)。
考虑到训练尺寸和测试尺寸之间的巨大差异(discrepancy)造成的性能下降，用固定S训练的模型使用三种大小的图片来测试，其大小接近训练大小：$ Q = {S-32, S, S+32} $。
同时，训练时候的尺寸变化使得网络在测试的时候可以应用到更广的尺寸上，所以用$ S \in [S_{min}, S_{max}] $训练的网络使用$ Q = {S_{min}, 0.5*(S_{min}+S_{max}), S_{max}} $来测试。
结果如下表4，表明在测试的时候使用尺寸变化可以得到更好的性能(相比于用单尺寸测试同样的网络，如表3所示)。
正如之前所述，最深的网络(D和E)，训练的时候使用尺寸变化比固定最小边S更好。
在验证集上我们最好的单个网络性能是24.8%/7.5%的top-1错误和top-5的错误(在表4中使用粗体高亮)。
在测试集上配置E实现了7.3%的top-5错误。  
![avatar](/images/posts/2019-03-05/vgg_table4.png)
+ 4.3多裁剪评估  
在表5中我们比较了dense Convnet evaluation和多裁剪评估(详见Sect. 3.2)。
我们也通过平均它们的soft-max输出来来评估两种评估技术的互补性(complementarity)。
正如我们所见，多裁剪评估略优于dense evaluation，还有两种真的具有互补性，两者结合优于它们本身。
正如上面所述，我们假设这是因为对卷积边界条件的不同处理。  
![avatar](/images/posts/2019-03-05/vgg_table5.png)  
+ 4.4卷积网络融合  
到目前为止，我们已经评估了单个卷积网络模型到性能。
在这个实验部分，我们通过平均它们的soft-max类后验来联合几种模型的的输出。
这样能提升性能是因为模型的互补性，这种方法在2012 (Krizhevsky et al., 2012) and 2013 (Zeiler & Fergus, 2013; Sermanet et al., 2014)ILSVRC名列前茅的提交中使用过。  
结果如表6所示。到ILSVRC提交时为止(by the time of)我们仅训练了单尺寸的网络，还有多尺寸模型D(仅通过微调全连接层而不是所有的层)。
7个网络集成实现了7.3%的ILSVRC测试错误。
在提交之后，我们考虑了一种仅用两个性能最好的多尺寸模型的集成(配置D和E)，使用dense evaluation测试集成模型可以将测试错误减少至7.0%，如果使用联合dense evaluation和多裁剪测试集成模型将测试错误减少到6.8%。
相对地，我们最好的单网络实现了7.1%错误（模型5，表5）。  
![avatar](/images/posts/2019-03-05/vgg_table6.png)
+ 同最先进技术的比较  
最后，我们在表7中同最先进的技术比较我们的结果。在ILSVRC-2014 challenge (Russakovsky et al., 2014)的分类任务中，我们“VGG”队伍使用7个模型的集成以7.3%的测试错误获得了第二名。
在提交之后，我们用两个模型的集成把错误率降至6.8%。  
从表7可见，我们的深度卷积网络明显优于前几代模型，那些在ILSVRC-2012和ILSVRC-2013比赛中取得最好结果的模型。
相比于分类任务的获胜者（GoogLeNet with 6.7% error）我们的结果也是富有竞争力的，并且明显地优于ILSVRC-2013的获胜者Clarifai，
该算法在有外面的训练数据的情况下可以实现11.2%的错误率，在没有外面训练数据的情况下实现11.7%的错误率。这是很出色的，鉴于我们的最好结果仅是靠两个模型的融合实现的，这明显少于过去大部分的ILSVRC提交。
在单网络性能方面，我们的网络实现了最好的结果(7.0%测试错误率)，比单个GoogLeNet好0.9%。
尤其，我们没有跳脱LeCun et al. (1989)提出的经典卷积神经网络架构，而是通过增加它的深度。  
![avatar](/images/posts/2019-03-05/vgg_table7.png)
+ 结论  
在本工作中我们评估了针对大规模图像识别的极深的卷积神经网络(多达19层)。
这反映了表征深度有利于分类准确度并且使用以加深传统度卷积网络架构(LeCun et al., 1989; Krizhevsky et al., 2012)的方式可以实现ImageNet挑战赛数据集的最先进性能。
在附录中，我们也展示了我们的模型也很好的泛化到各种任务和数据集(a wide range of),我们的模型比肩甚至超越更复杂的图像识别网络架构，这些架构构建在更浅的图像表征上。
我们的结果再次证明在可视表征中深度的重要性。